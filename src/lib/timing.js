...
const { setTimeout, clearTimeout, setImmediate } = require("timers-browserify");
...
export class Latency {
  ...
  showPartial() {
    ...
    if (this.options.maxRequests) {
      percent =
        " (" +
        Math.round((100 * this.totalRequests) / this.options.maxRequests) +
        "%)";
    }
    log.info(
      "Requests: %s%s, requests per second: %s, mean latency: %s ms",
      this.totalRequests,
      percent,
      results.rps,
    ...
    if (this.totalErrors) {
      percent =
        Math.round((100 * 10 * this.totalErrors) / this.totalRequests) / 10;
      log.info(
        "Errors: %s, accumulated errors: %s, %s% of total requests",
        this.partialErrors,
        this.totalErrors,
        percent
      );
  ...
  isFinished() {
    log.debug(
      "Total requests %s, max requests: %s",
      this.totalRequests,
      this.options.maxRequests
    );
    if (
      this.options.maxRequests &&
      this.totalRequests >= this.options.maxRequests
    ) {
  ...
  show() {
    ...
    if (this.options.requestsPerSecond) {
      log.info(
        ...
        this.options.requestsPerSecond * this.options.concurrency
      );
    }
    log.info("");
