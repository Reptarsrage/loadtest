...
const { setTimeout, clearTimeout } = require("timers-browserify");
...
export function loadTest(options, callback) {
  ...
  if (
    ...
    log.error(
      "Invalid URL %s, must be http://, https:// or ws://",
      options.url
    );
    return;
  }
...
class Operation {
  ...
  start() {
    ...
    this.showTimer = new timing.HighResolutionTimer(SHOW_INTERVAL_MS, () =>
      this.latency.showPartial()
    );
  ...
  callback(error, result, next) {
    ...
    if (this.running && this.options.statusCallback) {
