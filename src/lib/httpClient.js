...
class HttpClient {
  ...
  init() {
    ...
    if (this.params.body) {
      ...
      this.options.headers["Content-Type"] =
        this.params.contentType || "text/plain";
    }
    if (this.params.cookies) {
      if (Array.isArray(this.params.cookies)) {
        ...
        console.error(
          "Invalid cookies %j, please use an array or a string",
          this.params.cookies
        );
      }
    }
  ...
  start() {
    ...
    this.requestTimer = new timing.HighResolutionTimer(interval, () =>
      this.makeRequest()
    );
  }
  ...
  makeRequest() {
    ...
    if (typeof this.params.requestGenerator == "function") {
      request = this.params.requestGenerator(
        this.params,
        this.options,
        lib.request,
        this.getConnect(id, requestFinished, this.options)
      );
    } else {
      request = lib.request(
        this.options,
   ...
   */
  getConnect(id, callback, options) {
    let body = "";
    return connection => {
